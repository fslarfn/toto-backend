// ==========================================================
// 🚀 SERVER.JS — KODE LENGKAP (DENGAN PERBAIKAN)
// ==========================================================

/**
 * Full-featured server for ERP TOTO ALUMINIUM
 * - Express + Socket.IO (Configured for Railway)
 * - Postgres (pg Pool)
 * - JWT auth + refresh
 * - Multer upload
 * - Safe DB helper
 * - Real-time "wo_update_cell" listener
 */
...
const JWT_SECRET =
  process.env.JWT_SECRET || "kunci-rahasia-super-aman-untuk-toto-app";
const DATABASE_URL =
  process.env.DATABASE_URL ||
  process.env.FALLBACK_DATABASE_URL ||
  "postgresql://postgres:password@localhost:5432/erp_toto";

// ==========================================================
// ✅ PERBAIKAN 1: KONFIGURASI SOCKET.IO
// ==========================================================
// Create HTTP server
const httpServer = http.createServer(app);
// Create Socket.IO server dengan konfigurasi yang benar
const io = new Server(httpServer, {
  cors: {
    origin: "*",
    methods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"],
    // Opsi 'transports' yang salah sudah dihapus dari sini
  },

  // Opsi ini harus di level utama, BUKAN di dalam cors
  transports: ["websocket", "polling"],
  allowUpgrades: true,

  // Ini membuat koneksi lebih stabil di hosting seperti Railway
  pingInterval: 10000, // Server mengirim "ping" tiap 10 detik
  pingTimeout: 5000, // Klien punya 5 detik untuk merespon "pong"
});
// ==========================================================
...
const pool = new Pool({
  ...
  ssl:
    DATABASE_URL.includes("railway") || DATABASE_URL.includes("heroku")
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    const r = await safeQuery("SELECT * FROM users WHERE username = $1", [
      username,
    ]);
...
app.put("/api/user/change-password", authenticateToken, async (req, res) => {
  try {
    ...
    if (!oldPassword || !newPassword)
      return res
        .status(400)
        .json({ message: "Password lama & baru diperlukan." });

    const r = await safeQuery("SELECT password_hash FROM users WHERE id = $1", [
      req.user.id,
    ]);
    if (!r || r.rows.length === 0)
      return res.status(404).json({ message: "User tidak ditemukan." });
    ...
    await safeQuery("UPDATE users SET password_hash = $1 WHERE id = $2", [
      hashed,
      req.user.id,
    ]);
...
app.get("/api/dashboard", authenticateToken, async (req, res) => {
  try {
    ...
    const [summaryRes, statusRes] = await Promise.all([
      safeQuery(summaryQuery, [month, year]),
      safeQuery(statusQuery, [month, year]),
    ]);

    res.json({
      summary: (summaryRes.rows && summaryRes.rows[0]) || {
        total_rupiah: 0,
        total_customer: 0,
      },
...
app.post("/api/workorders", authenticateToken, async (req, res) => {
  try {
    const { tanggal, nama_customer, deskripsi, ukuran, qty, harga, no_inv } =
      req.body || {};
...
app.get("/api/workorders/chunk", authenticateToken, async (req, res) => {
  try {
    ...
    if (!month || !year) {
      return res
        .status(400)
        .json({ message: "Parameter month dan year wajib diisi." });
    }
  ...
  try {
    ...
    if (status) {
      if (status === "belum_produksi")
        where += ` AND (di_produksi = 'false' OR di_produksi IS NULL)`;
...
// Update (autosave) - Ini adalah FALLBACK jika Socket.IO gagal
app.patch("/api/workorders/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (fields.length === 0)
      return res
        .status(400)
        .json({ message: "Tidak ada kolom valid untuk diupdate." });

    values.push(id);
    const q = `UPDATE work_orders SET ${fields.join(
      ", "
...
app.patch("/api/workorders/:id/status", authenticateToken, async (req, res) => {
  try {
    ...
    if (!columnName)
      return res.status(400).json({ message: "columnName tidak ada" });
...
app.post("/api/workorders/mark-printed", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      message: `Berhasil menandai ${r.rowCount} Work Order sebagai printed.`,
      updated: r.rows,
    });
  } catch (err) {
...
app.delete("/api/workorders/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const r = await safeQuery("DELETE FROM work_orders WHERE id = $1 RETURNING *", [
      id,
    ]);
    if (r.rowCount === 0)
      return res.status(404).json({ message: "Work order tidak ditemukan." });
...
/// =======================================================
// ✅ STATUS BARANG — Ambil Semua Data Work Orders Lengkap (Versi Aman)
// =======================================================
app.get("/api/status-barang", authenticateToken, async (req, res) => {
  try {
    ...
    // cek dulu apakah kolom bulan & tahun tersedia
    // jika tidak, kita filter berdasarkan tanggal (bulan dari tanggal)
    const query = `
      SELECT
        id,
...
app.post("/api/karyawan", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal menambah karyawan.", error: err.message });
  }
});
...
app.delete("/api/karyawan/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const r = await safeQuery("DELETE FROM karyawan WHERE id = $1 RETURNING *", [
      id,
    ]);
    if (r.rowCount === 0)
      return res.status(404).json({ message: "Karyawan tidak ditemukan." });
...
app.post("/api/payroll", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (
      !karyawan_id ||
      potongan_kasbon === undefined ||
      potongan_kasbon === null
    )
      return res
        .status(400)
        .json({ message: "karyawan_id & potongan_kasbon diperlukan." });
    ...
    if (r.rowCount === 0) {
      ...
      return res
        .status(404)
        .json({ message: "Karyawan tidak ditemukan saat update kasbon." });
    }
    ...
    res
      .status(500)
      .json({ message: "Gagal memproses payroll.", error: err.message });
  } finally {
    client.release();
  }
});
...
app.post("/api/stok/update", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!bahan_id || !tipe || jumlah === undefined || jumlah === null)
      throw new Error("bahan_id, tipe, jumlah diperlukan.");

    const bahanRes = await client.query(
      "SELECT * FROM stok_bahan WHERE id = $1 FOR UPDATE",
      [bahan_id]
    );
    ...
    await client.query(
      "UPDATE stok_bahan SET stok = $1, last_update = NOW() WHERE id = $2",
      [stokSesudah, bahan_id]
    );
    await client.query(
...
app.post("/api/surat-jalan", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!tipe || !items)
      return res.status(400).json({ message: "tipe dan items diperlukan." });
...
app.post("/api/keuangan/transaksi", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!tanggal || jumlah === undefined || !tipe || !kas_id)
      return res
        .status(400)
        .json({ message: "tanggal, jumlah, tipe, kas_id diperlukan." });

    const jumlahNumeric = parseFloat(jumlah);
    const kasRes = await client.query(
      "SELECT * FROM kas WHERE id = $1 FOR UPDATE",
      [kas_id]
    );
    ...
    const saldoSesudah =
      tipe === "PEMASUKAN"
        ? saldoSebelum + jumlahNumeric
        : saldoSebelum - jumlahNumeric;

    await client.query("UPDATE kas SET saldo = $1 WHERE id = $2", [
      saldoSesudah,
      kas_id,
    ]);
    await client.query(
...
app.get("/api/users", authenticateToken, async (req, res) => {
  try {
    ...
    const r = await safeQuery(
      "SELECT id, username, phone_number, role, COALESCE(subscription_status, 'inactive') AS subscription_status FROM users ORDER BY id ASC"
    );
    res.json(r.rows || []);
  } catch (err) {
...
// ======================================================
// ✅ PERBAIKAN 2: LOGIKA REAL-TIME SOCKET.IO
// ======================================================

io.on("connection", (socket) => {
  console.log(`🟢 User terhubung via Socket.IO: ${socket.id}`);

  // ========= WORK ORDERS (Event dari HTTP 'Create' & 'Delete') =========
  // Ini tetap dipakai untuk "Create" (dari app.post) dan "Delete"
  socket.on("wo_created", (data) => {
    console.log("📡 [Socket] WO Baru dibuat (broadcast):", data?.id || "(tanpa ID)");
    socket.broadcast.emit("wo_created", data);
  });

  socket.on("wo_deleted", (data) => {
    console.log("📡 [Socket] WO Dihapus (broadcast):", data?.id || "(tanpa ID)");
    socket.broadcast.emit("wo_deleted", data);
  });

  // ========= INI LISTENER BARU UNTUK REAL-TIME EDIT =========
  // Ini mendengarkan event 'wo_update_cell' dari app.js
  socket.on("wo_update_cell", async (data) => {
    try {
      ...
      if (r.rows && r.rows.length > 0) {
        ...
        // 'app.js' akan menangkap 'wo_updated' dan update tabel
        io.emit("wo_updated", updated);
      }
    } catch (err) {
      console.error("❌ [Socket] Gagal update wo_update_cell:", err.message);
      // Kirim error kembali ke pengirim (opsional)

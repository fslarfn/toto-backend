// ==========================================================
// 🚀 SERVER.JS (VERSI FINAL - UTUH & Diperbaiki)
// ==========================================================
...
// Fallback DB (gunakan DATABASE_URL jika tersedia)
...
const io = new Server(httpServer, {
  cors: {
    // Sesuaikan origin untuk produksi; tambahkan domain frontend-mu
    origin: [
      "https://erptoto.up.railway.app",
      "http://localhost:8080",
      "http://localhost:5000",
      "*" // Optional: bila perlu sementara; tapi disarankan dicabut di produksi
    ],
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // req.user mungkin belum terisi ketika middleware multer dipasang sebelum auth.
    // Namun pada route-profile kita letakkan authenticateToken sebelum upload.
...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
  ...
  try {
    const summaryQuery = `
      SELECT
        COALESCE(SUM(
          NULLIF(REPLACE(NULLIF(TRIM(ukuran), ''), ',', '.')::numeric, 0) *
          NULLIF(REPLACE(NULLIF(TRIM(qty), ''), ',', '.')::numeric, 0) *
          NULLIF(REPLACE(NULLIF(TRIM(harga), ''), ',', '.')::numeric, 0)
        ), 0) AS total_rupiah,
    ...
    const statusQuery = `
      SELECT
        ...
        COUNT(*) FILTER (WHERE di_warna = 'true' AND (siap_kirim = 'false' OR siap_kirim IS NULL)) AS di_warna,
...
app.patch('/api/workorders/:id', authenticateToken, async (req, res) => {
  try {
    ...
    for (const [key, val] of Object.entries(filteredUpdates)) {
      ...
      // Jika nilai boolean, simpan 'true'/'false' string jika schema-mu simpan string; 
      // jika schema boolean, kirim boolean tanpa ubah.
...
app.patch('/api/workorders/:id/status', authenticateToken, async (req, res) => {
  try {
    ...
    // Simpan sebagai boolean (jika DB boolean) atau string 'true'/'false' tergantung schema.
...
// 7) GET /api/workorders (untuk dashboard)
app.get('/api/workorders', authenticateToken, async (req, res) => {
  try {
    ...
    // Filter minimal agar frontend tidak menerima entry kosong jika memang diinginkan
...
app.delete('/api/workorders/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Kembalikan info penghapusan
    res.json({ message: 'Work order dihapus.', id: id, deletedRow: r.rows[0] });
  } catch (err) {
...
app.delete('/api/karyawan/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM karyawan WHERE id = $1 RETURNING *', [id]);
    if (result.rowCount === 0) return res.status(404).json({ message: 'Karyawan tidak ditemukan.' });
    res.json({ message: 'Karyawan dihapus.', deleted: result.rows[0] });
  } catch (err) {
...
app.get('/api/invoices/summary', authenticateToken, async (req, res) => {
  try {
    ...
    const query = `
      SELECT
        COALESCE(SUM(NULLIF(REPLACE(NULLIF(TRIM(ukuran), ''), ',', '.')::numeric * NULLIF(REPLACE(NULLIF(TRIM(qty), ''), ',', '.')::numeric * NULLIF(REPLACE(NULLIF(TRIM(harga), ''), ',', '.')::numeric)),0), 0) AS total,
        COALESCE(SUM(CASE WHEN pembayaran = 'true' THEN NULLIF(REPLACE(NULLIF(TRIM(ukuran), ''), ',', '.')::numeric * NULLIF(REPLACE(NULLIF(TRIM(qty), ''), ',', '.')::numeric * NULLIF(REPLACE(NULLIF(TRIM(harga), ''), ',', '.')::numeric) ELSE 0 END), 0) AS paid
      FROM work_orders
...
app.get('/api/users', authenticateToken, async (req, res) => {
  try {
    // contoh validasi sederhana: hanya user 'faisal' yang boleh
